...
j=np.argmax(diff)
t=(HC[j,2]+HC[j+1,2])/2
cat=fcluster(HC, n-j, criterion='maxcluster')
labels_clusters=['C'+str(i)for i in cat]

#CALCUL PARTITIE OARECARE
kmeans=KMeans(n_clusters=5, n_init=10)
k_labels=['C'+str(i)for i in kmeans.fit_predict(x)]
...
#TRASARE PLOT DENDOGRAMA EVIDENTIEREA PARTITIEI PRIN CULOARE
#PARTITIE OPTIMALA
...
plt.scatter(x[:.0], x[:,1], c=silhouette_opt_obs, cmap='veridis')
plt.show()

plt.figure(figsize=(12,12))
plt.title('Silhouette partitie 5 clusteri')
silhouette_fixed_obs=silhouette_samples(x, kmeans.fit_predict(x))
plt.scatter(x[:,0], x[:,1], c=silhouette_fixed_obs, cmap='veridis')
plt.show()

#TRASARE HISTOGRAME CLUSTERI PT FIECARE VARIABILA OBSERVATA
variabila='2015'
...
plt.scatter(pca_scores[:,0], pca_scores[:,1], c=cat, cmap='veridis')
plt.colorbar()
plt.show()
